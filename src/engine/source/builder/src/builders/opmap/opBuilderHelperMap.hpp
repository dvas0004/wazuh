...
/**
 * @brief Maps an IANA IP protocol name to its numeric code (0..255).
 ...
 * Normalization: case-insensitive, spaces/underscores -> '-', and common aliases
 * are accepted (icmpv6->ipv6-icmp, udp-lite->udplite, ip-in-ip->ipip).
 *
 ...
 * @return MapOp producing a JSON number in [0,255] on success.
 *
 * Fails at runtime if the reference is missing, not a string, or the name is unknown.
 *
